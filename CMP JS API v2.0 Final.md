...
The [`window.postMessage()`](https://developer.mozilla.org/en-US/docs/Web/API/Window/postMessage) method may be used from a child iframe to make requests from a parent or any ancestor's frame's CMP API. To locate an ancestor frame capable of responding to `postMessage()` CMP API calls search for an ancestor frame that has a child frame named `'__tcflocator'` (see [sample code](#is-there-a-sample-iframe-script-call-to-the-cmp-api)).
...
(function() {
 ...
 /**
  * Set up a __tcfapi function to do the postMessage and map the callback.
